...
/** Routes the lead by job type — demo stand-in for CRM / dispatch queues */
function routeQueue(service: ServiceType): string {
  switch (service) {
    ...
    case "panel":
    case "generator":
      return "service-estimators";
    case "ev":
      return "ev-install-desk";
    case "lighting":
    case "outlets":
      return "residential-board";
  }
}
...
export async function POST(request: Request) {
  ...
  const ticketId = `VDE-${Date.now().toString(36).toUpperCase()}`;
  ...
  return NextResponse.json({
    ...
    urgency: service === "emergency" ? "emergency" : "scheduled",
    message:
      service === "emergency"
        ? "Routed to priority dispatch. In production this pages the on-call electrician."
